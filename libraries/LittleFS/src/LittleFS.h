/*
 ...
 Ported to Ai-WB2-12F (BL602):
   - The filesystem region is the "media" partition, resolved lazily from the
     ...
     construction of the global `LittleFS` never touches flash. FS_PHYS_ADDR
     etc. (ESP8266 flash_hal.h) therefore don't exist here.
   - lfs is vendored into this library (src/lfs.*), not ../lib/littlefs.
 */
 ...
 * "media" partition, same area SPIFFS uses). Defined in LittleFS.cpp. */
extern void wb2_littlefs_region(uint32_t *start, uint32_t *size);
...
public:
    ...
    bool begin() override {
        ...
        /* BL602: resolve the "media" partition lazily on first use. */
        if (!_size || !_blockSize) {
            wb2_littlefs_region(&_start, &_size);
            if (!_size || !_blockSize) {
                DEBUGV("LittleFS region unresolved (size=%d block=%d)", _size, _blockSize);
                return false;
            }
            _setupConfig();
        }
...
protected:
    ...
    /* Build the lfs_config from the (possibly lazily-resolved) geometry.
     * Values mirror the ESP8266 core: 64-byte read/prog/cache units are
     * multiples of the BL602 flash page-program granularity. */
    void _setupConfig() {
