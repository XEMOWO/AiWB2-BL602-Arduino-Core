...
{
    BitOrder order = settings.bitOrder();
...
/* ESP8266 void* transfer overload: same as the uint8_t* form. */
void SPIClass::transfer(void *buf, uint16_t count)
{
    transfer((uint8_t *)buf, (size_t)count);
}

/* ESP8266 full-duplex transfer: sends `out` (0xFF dummy bytes when NULL),
 * reads `size` bytes into `in` (discarded when NULL). */
void SPIClass::transferBytes(const uint8_t *out, uint8_t *in, uint32_t size)
{
    ...
    if (out) {
        ...
        memset(tx, 0xFF, size); /* read-only transfer: clock out dummy bytes */
    }
    if (_bitOrder == LSBFIRST) {
        for (i = 0; i < size; i++) {
            tx[i] = _swap_bits(tx[i]);
        }
    }
    ...
    if (SPI_SendRecv_8bits(s_port, tx, rx, size, SPI_TIMEOUT_ENABLE) != SUCCESS) {
        _faults++;
    }
    if (_bitOrder == LSBFIRST) {
        for (i = 0; i < size; i++) {
            rx[i] = _swap_bits(rx[i]);
        }
    }
    if (in) {
        memcpy(in, rx, size); /* else: received bytes discarded */
    }
    if (own_cs) {
        _cs(HIGH);
    }
...
{
    _bitOrder = (order == LSBFIRST) ? LSBFIRST : MSBFIRST;
}
