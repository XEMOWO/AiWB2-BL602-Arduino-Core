...
/*
    EthernetCompat.h — classic Arduino Ethernet API over a LwipIntfDev, for the
    Ai-WB2-12F (BL602).

    Compile-compatible port of the ESP8266 EthernetCompat.h. `ArduinoEthernet`
    adds the Arduino Ethernet shield API (begin(mac, ip, dns, gateway, subnet)
    with legacy argument order, hardwareStatus, maintain, MACAddress) on top of
    LwipIntfDev. On the WB2 the device is a no-op, so Ethernet.begin() always
    reports failure and hardwareStatus() stays EthernetNoHardware — the legacy
    examples print "Ethernet shield was not found" and idle.
*/
...
public:
                  ...
                  IPAddress arg3 = IPADDR_NONE)
    {
        ...
        if (ret)
        {
            ...
            if (ret && !local_ip.isSet())
            {
                // Arduino API waits for DHCP answer (never reached on WB2:
                // begin() above always reports no hardware)
