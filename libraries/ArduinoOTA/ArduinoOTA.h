...
class ArduinoOTAClass
{
  public:
    ...
    //Sets the erase-config behaviour at reboot (OTAEraseConfig example). No-op
    //on WB2: there is no separate WiFi-settings region to erase.
    void setEraseConfig(ota_erase_cfg_t eraseConfig = OTA_ERASE_CFG_ABORT_ON_ERROR) { (void)eraseConfig; _eraseConfig = eraseConfig; }

    //Erases the WiFi settings and resets (OTASdkCheck example). On WB2 this is
    //just a reset — "only returns on fail", so never return on success.
    bool eraseConfigAndReset() { esp_restart(); return false; }
