...
bool DNSServer::enableForwarder(const String &domainName, const IPAddress &dns)
{
  ...
  if (_dns.isSet()) {
    /* BL602 lwIP has no DNS forwarder, so there is no request queue to build:
     * pretend the forwarder is on and let respondToRequest() answer unmatched
     * queries (wildcard / NXDOMAIN) instead of relaying them upstream. */
    _forwarder = true;
  }
  return _forwarder;
}
                     ...
                     const IPAddress &resolvedIP, const IPAddress &dns)
{
  ...
  enableForwarder(domainName, dns);
