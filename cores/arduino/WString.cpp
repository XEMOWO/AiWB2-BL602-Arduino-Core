...
/* F("...") — PROGMEM is a no-op here, so treat it as a plain C-string. */
String::String(const __FlashStringHelper *pstr)
    : _buf(NULL), _len(0), _cap(0)
{
    if (pstr) {
        concat((const char *)pstr);
    }
}
...
/* 64-bit variants of the above. A full 64-bit magnitude needs up to 65 digits
 * in base 2 (plus sign); keep the whole render on unsigned long long. */
unsigned char String::concat(long long num, unsigned char base)
{
    char tmp[67];
    int  i = 0;
    if (num < 0 && base == 10) {
    ...
    char digits[66];
    int  d = 0;
    unsigned long long v = (unsigned long long)num;
    if (v == 0) {
        digits[d++] = '0';
    }
    while (v) {
...
unsigned char String::concat(unsigned long long num, unsigned char base)
{
    char digits[66];
    int  d = 0;
...
{
    /* ESP8266-compatible constant-time comparison (digest auth etc.). */
    if (_len != s2._len) {
        return 0;
    }
    if (_len == 0) {
    ...
    unsigned char equalcond = (equalchars == _len);
...
/* Shared empty String (ESP8266 core also defines this in WString.cpp). */
