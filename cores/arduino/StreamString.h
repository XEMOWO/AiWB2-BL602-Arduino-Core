/*
 * StreamString.h — String as a Stream (ESP8266-compatible).
 *
 * Ported from cores/esp8266/StreamString.h of the esp8266/Arduino repo
 * (Copyright (c) 2020 D. Gauchard, LGPL 2.1+). Gives a String object that can
 * be read/written as a Stream — used by libraries that build responses into a
 * Stream (e.g. serving a buffer over HTTP) and by ESP8266's own WebServer.
 */
...
public:
    ...
    /* make the string consumed as the stream is read (default) */
    void setConsume()
    {
        peekPointer = -1;
    }

    /* reset the read cursor and allow rereading (opposite of setConsume()) */
