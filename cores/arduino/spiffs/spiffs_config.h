...
#include <string.h>
#include <stddef.h>
#include <stdint.h> /* int32_t/uint32_t for the s32_t... typedefs below */
#include <unistd.h>
...
// for debug
// #define SPIFFS_DBG printf
// #define SPIFFS_GC_DBG printf
// #define SPIFFS_CACHE_DBG printf
// #define SPIFFS_CHECK_DBG printf
// #define SPIFFS_API_DBG printf
...
/* Use the stdint spellings (int32_t, not `signed int`): under this SDK's
 * -mabi=ilp32f the SiGeR toolchain defines __INT32_TYPE__ as `long`, so a
 * bare `signed int` typedef would clash with lwIP's `typedef int32_t s32_t`
 * in any TU that includes both (SPIFFS + lwIP). Same types, identical tokens. */
