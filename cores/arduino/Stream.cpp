...
/* Default multi-byte read: fill via read(), returning count of bytes actually
 * read. Classes with buffered data (WiFiClient, S2Stream) override this. */
int Stream::read(uint8_t *buffer, size_t len)
{
    size_t got = 0;
    while (got < len) {
        int c = read();
