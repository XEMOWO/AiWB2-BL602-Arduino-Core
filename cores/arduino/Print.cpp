...
/* F() output. PROGMEM is a no-op on this XIP target, so the flash pointer is a
 * plain C-string; forward straight to the const char* overloads. */
...
/* PROGMEM-format printf. On this XIP target a flash string is a plain
 * C-string, so this is identical to printf(). */
int Print::printf_P(PGM_P format, ...)
{
    char buf[256];
    va_list ap;
    va_start(ap, format);
