/*
 ...
 Ported to Ai-WB2-12F (BL602):
   - Hal callbacks are the SDK's spiffs_read/write/erase_flash (bl_spiffs.c,
     hosal_flash_raw_* based), taking raw flash offsets.
   - The filesystem region is the "media" partition, resolved lazily from the
     boot2 partition table via bl_mtd (not linker symbols) so static
     construction of the global `SPIFFS` never touches flash.
   - CHECKFD uses wb2_runtime_panic() (freestanding.cpp).
 */
#ifndef spiffs_api_h
#define spiffs_api_h
...
extern "C" {
    ...
    #include "spiffs/bl_spiffs.h"
    #include <bl_mtd.h> /* C API, no extern "C" guards of its own */
};
#include "debug.h"

extern "C" void __attribute__((noreturn)) wb2_runtime_panic(void); /* freestanding.cpp */
...
/*
 * Resolve the SPIFFS backing region from the boot2 partition table.
 * The media partition ("media" / BL_MTD_PARTITION_NAME_ROMFS) is a dedicated
 * app-data area (0x1A2000 / 0x47000 in the 2M layout). Called lazily so it
 * only reads flash once the user calls SPIFFS.begin().
 */
static void wb2_spiffs_region(uint32_t *start, uint32_t *size)
{
...
public:
    ...
    bool info(FSInfo& info) override
    {
        ...
        /* u32_t, not uint32_t: spiffs uses unsigned int, which differs from
           uint32_t (unsigned long) on RISC-V */
        u32_t totalBytes, usedBytes;
...
protected:
    ...
    {
        if (_size == 0) {
            wb2_spiffs_region(&_start, &_size);
        }
    }
    ...
    bool _tryMount()
    {
        ...
        config.hal_read_f       = &spiffs_read_flash;
        config.hal_write_f      = &spiffs_write_flash;
        config.hal_erase_f      = &spiffs_erase_flash;
        config.phys_size        = _size;
        config.phys_addr        = _start;
        config.phys_erase_block = 4096; /* BL602 flash sector */
...
#define CHECKFD() while (_fd == 0) { wb2_runtime_panic(); }
