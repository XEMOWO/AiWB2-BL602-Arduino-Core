...
void uart_rx_notify(void *arg)
{
    ...
    while ((c = bl_uart_data_recv(s->_uart_id)) >= 0) {
        ...
        if (next == s->_rx_tail) {
            s->_rx_overrun = 1; /* ring full: drop the newest byte */
            break;
        }
...
/* ESP8266-core API: did the RX ring overflow since the last call? Clears the
 * flag so the caller sees each overrun once. */
...
{
    (void)en; /* BL602 console is already UART0 */
}

/* ESP8266 auto-baud detection. BL602 has no frame-error probe; report the
 * configured baud immediately so callers that gate on it keep running. */
...
{
    _baud = baud;
    /* ESP8266 sketches often spell UART0's "default" pins explicitly as
     * TX=GPIO1 / RX=GPIO3 (the Xtensa part's console pins). Both are flash
     * pins on BL602, so map them to the WB2 Serial defaults (GPIO16/GPIO7). */
    if (_uart_id == 0) {
        if (tx_pin == 1)  tx_pin = PIN_SERIAL0_TX;
        if (rx_pin == 3)  rx_pin = PIN_SERIAL0_RX;
    }
...
void HardwareSerial::begin(uint32_t baud, SerialConfig config, SerialMode mode,
                           uint8_t tx_pin, bool invert)
{
    (void)config;
    (void)mode;
    (void)invert; /* BL602 UART is fixed 8N1 full-duplex with no invert line */

    if (tx_pin == 0) {
        begin(baud); /* 0 = keep the port's default pins */
    } else {
        /* ESP8266 only gives a TX pin here; RX stays on the port default. */
        begin(baud, tx_pin, (_uart_id == 0) ? PIN_SERIAL0_RX : PIN_SERIAL1_RX);
    }
}
...
/* ESP8266 Serial.swap() moves UART0 to RX=GPIO13/TX=GPIO15. The WB2 module's
 * mux already picks the default UART0 pins at boot, and the SerialStress
 * loopback path (UCLBE) doesn't exist on BL602, so swapping would strand the
 * console for no benefit — keep the default pins. */
void HardwareSerial::swap(void)
{
    /* no-op: see header comment */
}

/* ESP8266 reallocates the RX ring in begin(); BL602's ISR ring is a fixed
 * power-of-two array, so the requested size is only reported back by
 * getRxBufferSize() (SerialStress logs it). Returns the real ring size, as
 * ESP8266 returns the size actually applied. */
