...
typedef struct {
    void (*fn)(void);      /* attachInterrupt handler */
    void (*fn_arg)(void *);/* attachInterruptArg handler */
...
/* Common registration: both attachInterrupt forms resolve a trigger, arm the
 * pin, and point the shared HOSAL dispatch at irq_entries[pin]. */
static void irq_attach(uint8_t pin, int mode)
{
    hosal_gpio_dev_t g;
...
void attachInterrupt(uint8_t pin, void (*userFunc)(void), int mode)
{
    ...
    irq_entries[pin].fn_arg = NULL;
    irq_entries[pin].arg = NULL;
    irq_attach(pin, mode);
}
...
void attachInterruptArg(uint8_t pin, void (*userFunc)(void *), void *arg, int mode)
{
    ...
    irq_entries[pin].fn_arg = userFunc;
    irq_entries[pin].arg = arg;
    irq_attach(pin, mode);
}
