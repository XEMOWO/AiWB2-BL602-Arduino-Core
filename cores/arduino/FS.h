/*
 ...
 Ported to Ai-WB2-12F (BL602): <time.h> is newlib's, no xipio/flash_hal
 dependencies; the SPIFFS backend is in spiffs_api.h/.cpp.
 */
...
/* Flash physical geometry used to construct a LittleFSImpl/SPIFFSImpl at a
 * given offset (FSTools and friends). Same values as the ESP8266 core's FS.h;
 * our spiffs_api.cpp / LittleFS.cpp define the same numbers privately. */
#ifndef FS_PHYS_PAGE
#define FS_PHYS_PAGE 256
#endif
#ifndef FS_PHYS_BLOCK
...
public:
    ...
    void close();
    operator bool() const;
    char* name() const; // non-const so classic `char* p = f.name();` sketches compile
